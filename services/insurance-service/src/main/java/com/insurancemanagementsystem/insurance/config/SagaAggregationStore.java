 ...
 * needed for insurance premium calculation.
 * <p>
 * Replaces the former in-memory {@code ConcurrentHashMap} implementation. The underlying
 * {@code saga_aggregations} table provides:
 * <ul>
 * <li>Crash resilience — state survives service restarts</li>
 * <li>Multi-instance safety — single DB source of truth</li>
 * <li>Transactional atomicity — {@link #retrieve(String)} uses SELECT FOR UPDATE + DELETE
 * within the same transaction as the outbox save, so a rollback preserves the state for
 * retry</li>
 * </ul>
 */
@Component
...
public class SagaAggregationStore {
	...
	/**
	 * Store an event payload for a given sagaId. Returns true when all three required
	 * events are present (saga is ready for calculation).
	 */
	public boolean storeAndCheckReady(String sagaId, String eventType, EventEnvelope envelope) {
		...
		if (ready) {
			...
			log.debug("SAGA state for sagaId={}: hasEstimation={}, hasCustomer={}, hasVehicle={}", sagaId,
					agg.getEstimationRequestPayload() != null, agg.getCustomerValidatedPayload() != null,
	...
	public SagaState peek(String sagaId) {
		UUID sagaUuid = UUID.fromString(sagaId);
		return repository.findById(sagaUuid).map(this::toState).orElse(null);
	}

	/**
	 * Retrieve and consume aggregation state (atomic find-and-delete within the current
	 * transaction). If the enclosing transaction rolls back, the DELETE is undone and the
	 * state remains available for retry.
	 */
	public SagaState retrieve(String sagaId) {
		UUID sagaUuid = UUID.fromString(sagaId);
		return repository.findByIdForUpdate(sagaUuid).map(agg -> {
	...
	public static class SagaState {
		...
		public boolean hasEstimationRequest() {
			return estimationRequest != null;
		}

		public boolean hasCustomerValidated() {
			return customerValidated != null;
		}

		public boolean hasVehicleValidated() {
			return vehicleValidated != null;
		}
