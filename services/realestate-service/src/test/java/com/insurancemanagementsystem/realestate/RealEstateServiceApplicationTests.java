...
class RealEstateServiceApplicationTests {
	...
	void getByIdViaRest_Returns200() {
		...
		restTestClient.get()
			.uri("/api/real-estate/{id}", realEstate.getId())
			.exchange()
			.expectStatus()
			.isOk()
			.expectBody()
			...
			.isEqualTo(true)
			.jsonPath("$.data.id")
			.isEqualTo(realEstate.getId().toString())
			.jsonPath("$.data.address")
			.isEqualTo("123 Main St")
			.jsonPath("$.data.cityId")
			.isEqualTo(34)
			...
			.isEqualTo("Concrete")
			.jsonPath("$.data.luxuryClassName")
			.isEqualTo("A")
			.jsonPath("$.data.usageTypeName")
	...
	void listViaRest_ReturnsAll() {
		...
		restTestClient.get()
			...
			.exists()
			.jsonPath("$.data.content[1].address")
			.exists();
	}
	...
	void create_WithoutAddress_Returns400() {
		...
		restTestClient.post()
			...
			.isEqualTo(false)
			.jsonPath("$.message")
			.value(msg -> assertThat((String) msg).containsIgnoringCase("address"));
	}
	...
	void create_WithInvalidSquareMeters_Returns400() {
		...
		restTestClient.post()
			...
			.isEqualTo(false)
			.jsonPath("$.message")
			.value(msg -> assertThat((String) msg).containsIgnoringCase("squareMeters"));
	}
