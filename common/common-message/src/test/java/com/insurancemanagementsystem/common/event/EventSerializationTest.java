...
class EventSerializationTest {
	...
	void shouldSerializeAndDeserializeCalculationFailedEvent() throws Exception {
		CalculationFailedEvent event = CalculationFailedEvent.builder().reason("No active insurance found").build();
	...
	void shouldSerializeAndDeserializeRealEstateDeletedEvent() throws Exception {
		RealEstateDeletedEvent event = RealEstateDeletedEvent.builder().realEstateId(UUID.randomUUID()).build();
	...
	void shouldSerializeAndDeserializeInsuranceDeletedEvent() throws Exception {
		InsuranceDeletedEvent event = InsuranceDeletedEvent.builder().insuranceId(UUID.randomUUID()).typeId(2).build();
	...
	void shouldHandleVehicleValidatedWithNullFields() throws Exception {
		...
		VehicleValidatedEvent event = VehicleValidatedEvent.builder().vehicleId(null).build();
	...
	void shouldHandleRealEstateValidatedWithNullFields() throws Exception {
		...
		RealEstateValidatedEvent event = RealEstateValidatedEvent.builder().realEstateId(null).build();
	...
	void shouldSerializeAndDeserializeEventEnvelope() throws Exception {
		...
		EstimationRequestedEvent convertedPayload = mapper.convertValue(deserialized.getPayload(),
	...
	/**
	 * Asserts complete round-trip for a SAGA event: JSON serialization, deserialization,
	 * and EventEnvelope wrapping/unwrapping.
	 */
	private <T extends BaseEvent> void assertSagaEventRoundTrip(T event, Class<T> clazz) throws Exception {
		...
		// Note: We compare JSON strings rather than using assertEquals(event,
		// deserialized)
		...
		assertEquals(mapper.writeValueAsString(deserialized), json, "Event JSON should survive round-trip");
		assertEquals(event.getEventType(), deserialized.getEventType(), "eventType should survive JSON round-trip");
		...
		assertEquals(sagaId, deserializedEnvelope.getSagaId(), "sagaId should survive envelope round-trip");
		assertEquals(expectedEventType, deserializedEnvelope.getEventType(),
				"eventType should survive envelope round-trip");
		assertEquals(traceId, deserializedEnvelope.getTraceId(), "traceId should survive envelope round-trip");
		assertNotNull(deserializedEnvelope.getTimestamp(), "timestamp should not be null in envelope");
		...
		// We verify that the payload converts to the correct type and preserves
		// eventType.
		T convertedPayload = mapper.convertValue(deserializedEnvelope.getPayload(), clazz);
		assertEquals(expectedEventType, convertedPayload.getEventType(), "Converted payload eventType should match");
	}

	/**
	 * Asserts basic round-trip for a domain event: JSON serialization, deserialization,
	 * and getEventType() verification.
	 */
	private <T extends BaseEvent> void assertDomainEventRoundTrip(T event, Class<T> clazz) throws Exception {
		...
		assertTrue(
				expectedEventType.endsWith("Created") || expectedEventType.endsWith("Updated")
						|| expectedEventType.endsWith("Deleted") || expectedEventType.endsWith("Changed"),
