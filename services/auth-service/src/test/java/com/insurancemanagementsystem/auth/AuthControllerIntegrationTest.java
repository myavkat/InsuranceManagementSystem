...
class AuthControllerIntegrationTest {
	...
	@Container
	static PostgreSQLContainer postgres = new PostgreSQLContainer("postgres:16-alpine").withDatabaseName("auth_db_test")
		.withUsername("test")
		.withPassword("test");
	...
	void register_shouldCreateUser() throws Exception {
		...
		String roles = objectMapper.readTree(result.getResponseBodyContent()).get("data").get("roles").toString();
	...
	void register_shouldRejectDuplicateUsername() {
		...
		RegisterRequest request = RegisterRequest.builder()
			...
			.password(VALID_PASSWORD)
			.build();
		restTestClient.post().uri("/api/auth/register").body(request).exchange().expectStatus().isOk();
	...
	void register_shouldValidateRequiredFields() {
		RegisterRequest request = RegisterRequest.builder().username("").email("not-an-email").password("12").build();
	...
	void login_shouldReturnTokens() throws Exception {
		...
		LoginRequest request = LoginRequest.builder().username("logintest").password(VALID_PASSWORD).build();
	...
	void login_shouldRejectInvalidPassword() {
		...
		LoginRequest request = LoginRequest.builder().username("badpw").password("wrongpassword").build();
	...
	void login_shouldRejectUnknownUsername() {
		LoginRequest request = LoginRequest.builder().username("nonexistent").password(VALID_PASSWORD).build();
	...
	void login_shouldLockAfterMaxFailedAttempts() throws Exception {
		...
		LoginRequest badRequest = LoginRequest.builder().username("locktest").password("wrongpassword").build();
		...
		ExchangeResult lockedResp = restTestClient.post()
			.uri("/api/auth/login")
			.body(badRequest)
			.exchange()
			.expectStatus()
		...
		String message = objectMapper.readTree(lockedResp.getResponseBodyContent()).get("message").asText();
	...
	void refresh_shouldRotateTokens() throws Exception {
		...
		ExchangeResult reuseResponse = restTestClient.post()
			.uri("/api/auth/refresh")
			.body(refreshRequest)
			.exchange()
			.expectStatus()
		...
		String reuseMessage = objectMapper.readTree(reuseResponse.getResponseBodyContent()).get("message").asText();
	...
	void publicKey_shouldReturnPem() throws Exception {
		ExchangeResult result = restTestClient.get()
	...
	private void registerUser(String username, String email, String password) {
		RegisterRequest request = RegisterRequest.builder().username(username).email(email).password(password).build();
		restTestClient.post().uri("/api/auth/register").body(request).exchange();
	}
	...
	private String loginAndGetAccessToken(String username, String password) throws Exception {
		...
		return objectMapper.readTree(result.getResponseBodyContent()).get("data").get("accessToken").asText();
	}
	...
	private String loginAndGetRefreshToken(String username, String password) throws Exception {
		...
		return objectMapper.readTree(result.getResponseBodyContent()).get("data").get("refreshToken").asText();
	}
