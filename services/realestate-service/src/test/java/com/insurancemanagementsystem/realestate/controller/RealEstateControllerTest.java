...
class RealEstateControllerTest {
	...
	void getAll_ReturnsPaginatedResponse() {
		...
		restTestClient.get()
			...
			.isArray()
			.jsonPath("$.data.content[0].address")
			.isEqualTo("123 Main St")
			.jsonPath("$.data.content[0].squareMeters")
			.isEqualTo(120.50);
	...
	void getById_ReturnsRealEstate() {
		...
		restTestClient.get()
			...
			.isEqualTo(34)
			.jsonPath("$.data.constructionTypeName")
			.isEqualTo("Concrete");
	...
	void getById_NotFound_Returns404() {
		...
		restTestClient.get()
			...
			.isEqualTo(false)
			.jsonPath("$.message")
			.isEqualTo("RealEstate not found with id: " + testId);
	...
	void create_WithValidBody_Returns201() {
		...
		restTestClient.post()
			...
			.isEqualTo(true)
			.jsonPath("$.message")
			.isEqualTo("RealEstate created successfully")
			.jsonPath("$.data.address")
			.isEqualTo("123 Main St")
			.jsonPath("$.data.squareMeters")
			.isEqualTo(120.50);
	...
	void create_WithoutAddress_Returns400() {
		...
		restTestClient.post()
			...
			.isEqualTo(false)
			.jsonPath("$.message")
			.value(msg -> org.assertj.core.api.Assertions.assertThat((String) msg).contains("address", "Address"));
	}
	...
	void create_WithInvalidSquareMeters_Returns400() {
		...
		restTestClient.post()
			...
			.isEqualTo(false)
			.jsonPath("$.message")
			.value(msg -> org.assertj.core.api.Assertions.assertThat((String) msg).contains("squareMeters"));
	}
	...
	void update_Returns200() {
		...
		restTestClient.put()
			...
			.isEqualTo(true)
			.jsonPath("$.message")
			.isEqualTo("RealEstate updated successfully")
			.jsonPath("$.data.address")
	...
	void delete_Returns200() {
		...
		restTestClient.delete()
			...
			.isEqualTo(true)
			.jsonPath("$.message")
			.isEqualTo("RealEstate deleted successfully");
	...
	void delete_NotFound_Returns404() {
		// Arrange
		doThrow(new EntityNotFoundException("RealEstate not found with id: " + testId)).when(realEstateService)
			.delete(testId);
	...
	void getConstructionTypes_Returns200() {
		// Arrange
		List<RealEstateConstructionType> types = List.of(new RealEstateConstructionType(1, "Concrete"),
				new RealEstateConstructionType(2, "Steel"));
		given(realEstateService.getConstructionTypes()).willReturn(types);

		// Act & Assert
		restTestClient.get()
			.uri("/api/real-estate/construction-types")
			.exchange()
			.expectStatus()
			.isOk()
			.expectBody()
	...
	void getLuxuryClasses_Returns200() {
		// Arrange
		List<RealEstateLuxuryClass> classes = List.of(new RealEstateLuxuryClass(1, "A"),
				new RealEstateLuxuryClass(2, "B"));
		given(realEstateService.getLuxuryClasses()).willReturn(classes);

		// Act & Assert
		restTestClient.get()
			.uri("/api/real-estate/luxury-classes")
			.exchange()
			.expectStatus()
			.isOk()
			.expectBody()
	...
	void getUsageTypes_Returns200() {
		// Arrange
		List<RealEstateUsageType> types = List.of(new RealEstateUsageType(1, "Residential"),
				new RealEstateUsageType(2, "Commercial"));
		given(realEstateService.getUsageTypes()).willReturn(types);

		// Act & Assert
		restTestClient.get()
			.uri("/api/real-estate/usage-types")
			.exchange()
			.expectStatus()
			.isOk()
			.expectBody()
