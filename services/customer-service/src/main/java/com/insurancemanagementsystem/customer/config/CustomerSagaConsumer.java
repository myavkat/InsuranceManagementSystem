...
public class CustomerSagaConsumer {
	...
	/**
	 * Validates the customer referenced in the estimation request and saves the result as
	 * an outbox event within the same transaction as the dedup marker. The outbox relay
	 * will later publish the event to Kafka atomically.
	 */
	private void handleEstimationRequested(EventEnvelope envelope, UUID sagaId, UUID traceId) {
		...
		transactionTemplate.executeWithoutResult(status -> {
			...
			// Save outbox event within the same transaction — the relay will publish it
			// afterward
	...
	/**
	 * Build a PENDING outbox event from the given saga context and event envelope. The
	 * payload is serialised to JSON so the relay can publish it without re-entering the
	 * consumer's classloader.
	 */
