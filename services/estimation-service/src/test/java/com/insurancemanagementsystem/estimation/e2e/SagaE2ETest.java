 ...
 * <p>
 * These tests spin up the estimation-service with real PostgreSQL and Kafka (via
 * Testcontainers) and simulate the full SAGA lifecycle by publishing events that other
 * services (customer, vehicle, insurance) would normally produce. This is the
 * <strong>Estimation-centric integration</strong> approach (Approach B from the plan) —
 * it validates the estimation service's complete SAGA coordination logic without needing
...
class SagaE2ETest extends AbstractKafkaIntegrationTest {
	 ...
	 * <p>
	 * Uses KafkaTemplate<String, String> to send pre-serialized JSON events, matching how
	 * other saga consumer tests publish. The StringSerializer writes the JSON as UTF-8
	 * bytes; the consumer's StringDeserializer reads them back as a String for jsonMapper
	 * deserialization.
	 */
	...
	void duplicateEvents_areSafelyIgnored() throws Exception {
		...
		List<EventCount> counts = List.of(new EventCount(EventConstants.CUSTOMER_VALIDATED, 1),
		...
		for (EventCount ec : counts) {
			long actualCount = sagaEventRepository.findBySagaIdAndEventType(sagaId, ec.eventType)
				.map(ignored -> 1L)
				.orElse(0L);
			assertThat(actualCount).as("Duplicate check for %s", ec.eventType).isEqualTo(ec.expectedCount);
		}
	}
	...
	void customerInvalidated_estimationRejected() throws Exception {
		...
		assertThat(outboxEvents)
			.anyMatch(e -> e.getSagaId().equals(sagaId) && e.getPayload().contains("EstimationFailed"));
	}
	...
	void calculationFailed_estimationRejected() throws Exception {
		...
		CalculationFailedEvent event = CalculationFailedEvent.builder().reason("Division by zero").build();
		publishSagaEvent(sagaId, UUID.randomUUID(), event);
	...
	void malformedJson_doesNotBlockConsumer() throws Exception {
		...
		UUID traceId = UUID.randomUUID();
		publishSagaEvent(sagaId, traceId,
				CustomerValidatedEvent.builder()
					.customerId(request.getCustomerId())
					.firstName("John")
					.lastName("Doe")
					.build());
		publishSagaEvent(sagaId, traceId,
				VehicleValidatedEvent.builder().vehicleId(request.getVehicleId()).plate("34ABC123").build());
		publishSagaEvent(sagaId, traceId, PremiumCalculatedEvent.builder().premium(new BigDecimal("1500.00")).build());
