...
/**
 * Tests that all 7 routes forward requests to the correct downstream paths. WireMock
 * stubs catch forwarded requests and verify the path rewriting.
 */
class GatewayRoutingTest extends BaseGatewayTest {

	private final String validToken = TestJwtTokenGenerator.createValidToken(UUID.randomUUID(), List.of("CUSTOMER"));
	...
	void referenceDataRouteForwardsCorrectly() {
		...
		client.get().uri("/api/reference-data/cities").exchange().expectStatus().isOk();
	}

	@Test
	@DisplayName("Unknown route returns 404")
	void unknownRouteReturns404() {
		client.get().uri("/api/nonexistent").exchange().expectStatus().isNotFound();
	}
