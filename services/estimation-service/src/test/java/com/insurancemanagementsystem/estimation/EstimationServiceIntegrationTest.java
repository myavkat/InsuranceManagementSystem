 ...
 * <p>
 * These tests exercise the real transaction lifecycle using Testcontainers for PostgreSQL
 * and Kafka, verifying that:
 * <ul>
 * <li>{@code EstimationService.create()} persists an {@link OutboxEvent} within the same
 * transaction</li>
 * <li>The {@link OutboxRelay} publishes outbox events to Kafka</li>
 * <li>The REST API end-to-end flow produces events in Kafka</li>
 * </ul>
 * <p>
 * <strong>Rollback scenario (manual):</strong> To verify that a transaction rollback
 * prevents outbox event publication, wrap {@code estimationService.create()} in a new
 * {@code @Transactional} method that throws a {@code RuntimeException} after the outbox
 * event is saved, then assert that no events reach Kafka and no outbox rows remain. This
 * is documented as a manual test scenario because {@code create()} itself does not
 * trigger a rollback on success.
 */
...
class EstimationServiceIntegrationTest {
	...
	void createEstimation_persistsOutboxEvent() {
		...
		assertThat(event.getPayload()).contains("EstimationRequested").contains(response.getSagaId().toString());
	}
	...
	void createEstimation_outboxRelayPublishesToKafka() {
		...
		// Kafka messages are Base64-encoded by Spring Cloud Stream's internal
		// serialization
	...
	void createEstimation_withRealEstateId_publishesEvent() {
		...
		when(insuranceServiceClient.getInsurance(realEstateInsuranceId))
			.thenReturn(new InsuranceServiceClient.InsuranceInfo(realEstateInsuranceId, "DASK (Doğal Afet Sigortası)",
					2, "Real Estate"));
	...
	void rollbackScenario_documentation() {
		log.warn("Rollback scenario: wrap create() in a @Transactional method that throws "
				+ "RuntimeException after save, then verify no events reach Kafka. " + "See javadoc on {} for details.",
	 ...
	 * {@code String} payload. The payload (a JSON string) is serialized as a JSON string
	 * literal ({@code "..."}) on the topic.
	 * <p>
	...
	private static List<ConsumerRecord<String, String>> pollForRecords(KafkaConsumer<String, String> consumer,
			Duration timeout) {
