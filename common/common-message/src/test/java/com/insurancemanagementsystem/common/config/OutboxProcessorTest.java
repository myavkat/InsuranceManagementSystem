...
class OutboxProcessorTest {
	...
	/**
	 * Helper: make transactionTemplate.executeWithoutResult() execute the callback
	 * immediately.
	 */
	...
	void publishFails_savesTwiceAndRetries() {
		...
		when(outboxEventRepository.findTop10ByStatusOrderByCreatedAtAsc(OutboxEvent.Status.PENDING.name()))
			.thenReturn(List.of(pending));
		doThrow(new RuntimeException("Kafka down")).when(messagePublisher).publish(anyString(), any());
	...
	void maxRetriesReached_staysFailed() {
		...
		when(outboxEventRepository.findTop10ByStatusOrderByCreatedAtAsc(OutboxEvent.Status.PENDING.name()))
			.thenReturn(List.of(pending));
		doThrow(new RuntimeException("Kafka still down")).when(messagePublisher).publish(anyString(), any());
	...
	void cleanupEvents_noStaleEvents_doesNothing() {
		...
		when(outboxEventRepository.findByStatusAndCreatedAtBefore(eq(OutboxEvent.Status.FAILED), any(Instant.class)))
			.thenReturn(List.of());
		when(outboxEventRepository.findByStatusAndCreatedAtBefore(eq(OutboxEvent.Status.PUBLISHING),
				any(Instant.class)))
			.thenReturn(List.of());
