...
public class EstimationSagaConsumer {
			...
			try {
				...
				observation.observe(() -> {
					switch (eventType) {
						...
						case EventConstants.ESTIMATION_FAILED -> handleEstimationFailed(envelope, sagaId);
	...
	private void handleCustomerValidated(EventEnvelope envelope, UUID sagaId, UUID traceId, JsonMapper jsonMapper) {
		...
		transactionTemplate.executeWithoutResult(status -> {
			...
			CustomerValidatedEvent event = jsonMapper.convertValue(envelope.getPayload(), CustomerValidatedEvent.class);
			log.info("Customer validated for sagaId={}: customerId={}, {} {}", sagaId, event.getCustomerId(),
	...
	private void handleVehicleValidated(EventEnvelope envelope, UUID sagaId, UUID traceId, JsonMapper jsonMapper) {
		...
		transactionTemplate.executeWithoutResult(status -> {
			...
			// Convert to typed event for logging
			VehicleValidatedEvent event = jsonMapper.convertValue(envelope.getPayload(), VehicleValidatedEvent.class);
			log.info("Vehicle validated for sagaId={}: vehicleId={}, plate={}", sagaId, event.getVehicleId(),
					event.getPlate());
		});
	}
	...
	private void handleRealEstateValidated(EventEnvelope envelope, UUID sagaId, UUID traceId, JsonMapper jsonMapper) {
		...
		transactionTemplate.executeWithoutResult(status -> {
			...
			// Convert to typed event for logging
			RealEstateValidatedEvent event = jsonMapper.convertValue(envelope.getPayload(),
					RealEstateValidatedEvent.class);
			log.info("Real estate validated for sagaId={}: realEstateId={}, address={}", sagaId,
					event.getRealEstateId(), event.getAddress());
		});
	}
	...
	private void handlePremiumCalculated(EventEnvelope envelope, UUID sagaId, UUID traceId, JsonMapper jsonMapper) {
		...
		transactionTemplate.executeWithoutResult(status -> {
			...
			// Convert payload
			PremiumCalculatedEvent event = jsonMapper.convertValue(envelope.getPayload(), PremiumCalculatedEvent.class);
			...
			estimationRepository.findBySagaId(sagaId).ifPresentOrElse(estimation -> {
				...
				log.info("Estimation {} waiting approval for sagaId={}: premium={}", estimation.getId(), sagaId,
						event.getPremium());
	...
	private void handleFailed(EventEnvelope envelope, UUID sagaId, UUID traceId, String reason, JsonMapper jsonMapper) {
		...
		transactionTemplate.executeWithoutResult(status -> {
			...
			estimationRepository.findBySagaId(sagaId).ifPresentOrElse(estimation -> {
				if (estimation.getStatus() != Estimation.Status.STARTED) {
					log.warn("Estimation {} is in status {} — cannot transition to REJECTED", estimation.getId(),
							estimation.getStatus());
					return;
				}

				// Serialize outbox event FIRST — if serialization fails, exception
				// propagates
				// and the estimation stays STARTED
				OutboxEvent outboxEvent = outboxEventSerializer.buildEstimationFailedOutboxEvent(sagaId, traceId,
						reason, eventType, EventConstants.ESTIMATION_SAGA);
				...
				try {
					...
					try {
						...
						estimation.setDetails(tools.jackson.databind.json.JsonMapper.builder()
							.build()
							.writeValueAsString(Map.of("reason", reason)));
					}
					catch (Exception ex) {
						// Absolute last resort — log and set a minimal valid JSON literal
						log.error("Safe fallback serialization also failed for sagaId={}: {}", sagaId, ex.getMessage(),
								ex);
