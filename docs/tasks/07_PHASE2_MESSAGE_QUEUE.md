...
2. **Provision Message Infrastructure**
   ...
   - All inter-service communication uses Kafka exclusively — no RabbitMQ.
...
3. **Build Common Message Library**
   ...
   - Abstractions: `MessagePublisher<T>`, `MessageListener<T>` with Kafka implementation.
...
4. **End-to-End SAGA Tests**
   - Integration test that spins up all services (or their messaging layers) with Testcontainers (PostgreSQL × N, Kafka).
...
5. **Implement Dead Letter Queue Handling**
   - Configure Kafka DLQ topic for failed SAGA event processing.
...
6. **Setup Distributed Tracing**
   ...
   - Configure trace propagation via Kafka headers.
