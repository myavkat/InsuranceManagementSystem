...
/**
 * Overrides Gateway's default 429 response to return standardized ErrorResponse JSON with
 * Retry-After header.
 *
 * This handler catches 429 errors thrown by the RequestRateLimiter filter, which can
 * manifest as either {@link ResponseStatusException} (when throwOnLimit is false) or
 * {@link HttpClientErrorException.TooManyRequests} (when throwOnLimit is true).
 */
@Configuration
...
public class RateLimitExceptionHandler implements ErrorWebExceptionHandler {
	...
	/**
	 * Checks whether the given throwable represents a rate-limit (429) error from the
	...
	private Mono<Void> handleRateLimit(ServerHttpResponse response) {
		...
		String errorJson = String.format(
				...
				retryAfter != null ? retryAfter : "60", java.time.Instant.now().toString());
