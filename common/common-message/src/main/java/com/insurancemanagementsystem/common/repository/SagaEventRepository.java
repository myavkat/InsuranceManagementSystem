...
public interface SagaEventRepository extends JpaRepository<SagaEvent, UUID> {
     ...
     * <p>
     * Returns the number of rows inserted: 1 if this is a new (sagaId, eventType) pair,
     * 0 if a row already exists (duplicate). The operation is atomic at the database
     * level — no TOCTOU race — and does <b>not</b> create a managed JPA entity, so
     * the persistence context stays clean even when the caller is inside a
     ...
     *         if called without an active transaction — this method must run inside
     *         the same transaction as any subsequent state changes (e.g. saving an
     *         outbox event), otherwise dedup-marking and the actual side effect can
     ...
     * {@code INSERT … ON CONFLICT DO NOTHING} — which is fully atomic at the DB
     * level and does not materialise a managed JPA entity. This avoids the
     * persistence-context poisoning that occurred when the previous implementation
     * used {@code saveAndFlush} + {@code DataIntegrityViolationException} catch
     * (Hibernate would re-flush the failed insert at commit time, corrupting the
     * surrounding transaction).
     *
     * @return {@code true} if this event was already processed (duplicate),
     *         {@code false} if new.
     */
