 ...
 * Spring Cloud Stream functional {@code Consumer<String>} bindings (SAGA consumers) use
 * binder-level retry and DLQ configuration in {@code application.yml} instead.
 * <p>
 * Retry sequence: 1s → 2s → 4s → 8s (5 total attempts, max 20s elapsed), then routes to
 * {@code dlq.saga} partition 0. Deserialization (poison-pill) failures are immediately
 * routed to DLQ without retry.
 */
@Configuration
@Slf4j
public class KafkaErrorHandlerConfig {
	...
	public CommonErrorHandler kafkaErrorHandler(KafkaTemplate<String, String> kafkaTemplate) {
		...
		DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(kafkaTemplate,
				(ConsumerRecord<?, ?> record, Exception exception) -> new TopicPartition("dlq.saga", 0));
		...
		// Deserialization (poison-pill) failures won't be fixed by retry — route to DLQ
		// immediately
	...
	/**
	 * Dedicated listener container factory for dlq-monitor-group. Uses a no-retry error
	 * handler to prevent infinite DLQ loops — if the monitor itself fails, the message is
	 * simply logged and committed (not re-published to dlq.saga).
	 */
	@Bean
