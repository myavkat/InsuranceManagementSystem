...
class EstimationServiceApplicationTests {
	...
	void createEstimationViaRest_verifyInDb() throws Exception {
		...
		UUID estimationId = UUID.fromString(objectMapper.readTree(body).get("data").get("id").asText());
	...
	void getById_NotFound_Returns404() {
		restTestClient.get()
			.uri("/api/estimations/{id}", UUID.randomUUID())
			.exchange()
			.expectStatus()
			.isNotFound()
			.expectBody()
	...
	void createEstimation_thenListReturnsOne() {
		...
		ExchangeResult postResult = restTestClient.post()
			.uri("/api/estimations")
			.body(request)
			.exchange()
			.returnResult();
		...
		restTestClient.get()
			...
			.isEqualTo("STARTED")
			.jsonPath("$.data.content[0].customerId")
			.isNotEmpty();
	}
