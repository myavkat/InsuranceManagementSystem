...
class InsuranceSagaConsumerTest {
	...
	void allThreeEvents_valid_shouldSaveOutboxPremiumCalculated() throws Exception {
		// Send events out of order: CustomerValidated first, then VehicleValidated, then
		// EstimationRequested
		CustomerValidatedEvent customerEvent = CustomerValidatedEvent.builder()
			...
			.lastName("Doe")
			.build();
		kafkaTemplate.send("estimation.saga", MAPPER.writeValueAsString(customerEvent.toEnvelope(sagaId, traceId)))
		...
		VehicleValidatedEvent vehicleEvent = VehicleValidatedEvent.builder()
			...
			.model("Corolla")
			.build();
		kafkaTemplate.send("estimation.saga", MAPPER.writeValueAsString(vehicleEvent.toEnvelope(sagaId, traceId)))
			.get(10, TimeUnit.SECONDS);
		...
		EstimationRequestedEvent estimationEvent = EstimationRequestedEvent.builder()
			...
			.build();
		kafkaTemplate.send("estimation.saga", MAPPER.writeValueAsString(estimationEvent.toEnvelope(sagaId, traceId)))
			.get(10, TimeUnit.SECONDS);
	...
	void customerInvalidated_shouldSaveOutboxCalculationFailed() throws Exception {
		...
		CustomerInvalidatedEvent invalidatedEvent = CustomerInvalidatedEvent.builder()
			...
			.build();
		kafkaTemplate.send("estimation.saga", MAPPER.writeValueAsString(invalidatedEvent.toEnvelope(sagaId, traceId)))
			.get(10, TimeUnit.SECONDS);
	...
	void eventsOutOfOrder_shouldStillCalculate() throws Exception {
		// Send events out of order: VehicleValidated first, then CustomerValidated, then
		// EstimationRequested
		VehicleValidatedEvent vehicleEvent = VehicleValidatedEvent.builder()
			.vehicleId(UUID.randomUUID())
			.plate("34ABC123")
	...
	void estimationFailedEvent_handlesDuplicates() throws Exception {
		...
		EstimationRequestedEvent estimationEvent = EstimationRequestedEvent.builder()
			...
			.build();
		kafkaTemplate
			.send("estimation.saga", MAPPER.writeValueAsString(estimationEvent.toEnvelope(failSagaId, traceId)))
			.get(10, TimeUnit.SECONDS);
		...
		CustomerValidatedEvent customerEvent = CustomerValidatedEvent.builder()
			...
			.lastName("Doe")
			.build();
		kafkaTemplate.send("estimation.saga", MAPPER.writeValueAsString(customerEvent.toEnvelope(failSagaId, traceId)))
			.get(10, TimeUnit.SECONDS);
		...
		VehicleValidatedEvent vehicleEvent = VehicleValidatedEvent.builder()
			...
			.model("Corolla")
			.build();
		kafkaTemplate.send("estimation.saga", MAPPER.writeValueAsString(vehicleEvent.toEnvelope(failSagaId, traceId)))
			.get(10, TimeUnit.SECONDS);
