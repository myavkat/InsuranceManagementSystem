...
/**
 * Base class for Gateway integration tests. Starts a WireMock server to simulate
 * downstream microservices. All routes are configured to forward to this WireMock server.
 */
...
public abstract class BaseGatewayTest {
	...
	/**
	 * Test-only CORS configuration using CorsWebFilter, which runs at the WebFilter level
	 * (before SCG route matching). This handles CORS preflight requests correctly with
	...
	static void startWireMock() {
		...
		// WireMock port is set as a system property so application-test.yml can reference
		// it
	...
	protected void stubPost(String path, int status, String body) {
		wireMockServer.stubFor(WireMock.post(WireMock.urlPathEqualTo(path))
			.willReturn(WireMock.aResponse()
				.withStatus(status)
				.withHeader("Content-Type", "application/json")
				.withBody(body)));
	...
	protected void stubOptionsWithCors(String path) {
		wireMockServer.stubFor(WireMock.options(WireMock.urlPathEqualTo(path))
			.willReturn(WireMock.aResponse()
				.withStatus(200)
